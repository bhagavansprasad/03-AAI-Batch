...
def invoke_jira_graph(pr_details: str, bugs: list):
    ...
    jira_return_value = jira_Ticket_graph.invoke(data)
    jira_tickets = jira_return_value['jira_tickets']
    log_ok(AGENT, f"JIRA returned  tickets={len(jira_tickets)}  →  {jira_tickets}")
    return jira_tickets
...
def jira_agent_node(state: OrchestraterData):
    ...
    # ⚠️  Currently returns early — remove the early return to enable Jira
    log_warn(AGENT, "JIRA_AGENT is currently DISABLED (early return in code)")
    log_step(AGENT, "To enable: remove the early 'return state' from jira_agent_node")
    return state   # <── remove this line to activate

    pr_details = state['pr_details']
    bugs       = state["llm_review_result"].get('bugs', [])

    log_step(AGENT, f"Bugs to create tickets for: {len(bugs)}")
    for i, b in enumerate(bugs, 1):
        log_step(AGENT, f"  Bug {i}: {b}")

    state['jira_ticket_details'] = invoke_jira_graph(pr_details, bugs)

    tickets = state['jira_ticket_details']
    log_ok(AGENT, f"Jira tickets created: {tickets}")
...
def graph_Builder():
    ...
    Ograph.add_node("JIRA_AGENT",        jira_agent_node)         # sync
